...
begin try
	...
	begin tran
		...
		set  Chance = 1 - (CSS.ConditionSum + 10 * C.Chance) / CS.Sume
		--select CS.SportEventID, C.ConditionID, CSS.ConditionSum, CS.Sume,  (isnull(CSS.ConditionSum, 0) + 10 * C.Chance) / CS.Sume
		from finance.Conditions C
			 join finance.Events E
				on E.EventID = C.EventID
